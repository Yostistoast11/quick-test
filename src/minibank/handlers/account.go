...
func getSessionLookup() func(string) (string, bool) {
	...
	if models.CassandraEnabled {
		return func(session string) (string, bool) {
			...
			sessions := models.CassandraSession.Query("SELECT username FROM sessions WHERE session = ?", session)
			switch err := sessions.Scan(&username); err {
                        case sql.ErrNoRows:
                                return "", false
			case nil:
...
func getSessionWriter() func(uuid.UUID, string) {
	...
	if models.CassandraEnabled {
		return func(session uuid.UUID, username string) {
			...
			models.CassandraSession.Query("INSERT INTO sessions(session, username, expiration) VALUES (?, ?, ?)",
			session.String(),
                                username,
...
func getSessionListLookup() func(string) UserSessions {
	...
	if models.CassandraEnabled {
		return func(username string) UserSessions {
			...
			rows := models.CassandraSession.Query("SELECT session FROM sessions WHERE username =?", username).Iter()
			var session string
